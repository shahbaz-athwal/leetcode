// Created by shahbaz_athwal at 2025/08/18 15:58
// leetgo: 1.4.15
...
function findOrder(numCourses: number, prerequisites: number[][]): number[] {
  ...
  for (const [course, prereq] of prerequisites) {
    if (graph.has(course)) graph.get(course)!.push(prereq);
    else graph.set(course, [prereq]);
  }
  ...
  function hasCycle(c: number) {
    if (visiting.has(c)) return true;
